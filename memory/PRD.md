...
## Next Tasks
- 2026-06: Added per-job editable "Decline / Denied Message" (Job.decline_message). Shown as a toast when the worker clicks Decline on the consent modal (PrivacyModal onDecline) AND when the browser blocks location (geoErr code 1) in CheckInPage. Editable in JobDialog consent box (data-testid job-decline-message). NOTE: decline_message initially only got added to Job model, not JobInput — API silently dropped it until JobInput field was added; both now present. Verified: persists via API + custom toast shown on modal decline (Playwright).
- Consolidate the two VPS clones (/root/contractor-checkin backend vs /root/h0sting frontend) into one folder so a single git pull updates both — user chose option (a): keep /root/contractor-checkin, archive /root/h0sting. Steps given to user; awaiting confirmation of completion.
